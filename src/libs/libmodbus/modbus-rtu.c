...
static int _modbus_rtu_receive(modbus_t *ctx, uint8_t *req)
{
    ...
    if (ctx_rtu->confirmation_to_ignore) {
        _modbus_receive_msg(ctx, req, MSG_CONFIRMATION, NULL);
        /* Ignore errors and reset the flag */
        ctx_rtu->confirmation_to_ignore = _FALSE_;
        rc = 0;
        if (ctx->debug) {
            printf("Confirmation to ignore\n");
        }
    } else {
        rc = _modbus_receive_msg(ctx, req, MSG_INDICATION, NULL);
        if (rc == 0) {
            /* The next expected message is a confirmation to ignore */
            ctx_rtu->confirmation_to_ignore = _TRUE_;
        }
    }
    return rc;
}
...
static int _modbus_rtu_flush(modbus_t *ctx)
{
    ...
    return (PurgeComm(ctx_rtu->w_ser.fd, PURGE_RXCLEAR) == _FALSE_);
#else
