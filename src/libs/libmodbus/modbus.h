...
enum {
    ...
    MODBUS_EXCEPTION_SLAVE_OR_SERVER_BUSY = 0x06,
    MODBUS_EXCEPTION_NEGATIVE_ACKNOWLEDGE = 0x07,
    MODBUS_EXCEPTION_MEMORY_PARITY = 0x08,
    MODBUS_EXCEPTION_NOT_DEFINED = 0x09,
    MODBUS_EXCEPTION_GATEWAY_PATH = 0x0A,
    MODBUS_EXCEPTION_GATEWAY_TARGET = 0x0B,
    MODBUS_EXCEPTION_MAX = 0x0C
};
/* TCP Exceptions */
enum {
  NO_SOCKET_CONNECTION = 0x1000,
  TCP_TIMEOUT_ERROR = 0x1001
};
...
#define EMBSOCKERR (MODBUS_ENOBASE + NO_SOCKET_CONNECTION)
#define EMBTIMEOUTERR (MODBUS_ENOBASE + TCP_TIMEOUT_ERROR)
...
typedef enum
{
 ...
 int modbus_send_raw_request(modbus_t *ctx, uint8_t **req, size_t req_size);

 int modbus_receive(modbus_t *ctx, uint8_t *req);

 // ДОБАВЛЕНИЕ ЧАСТИ ДЛЯ ИСПРАВЛЕНИЯ БАГА ПОЛУЧЕНИЯ ОТВЕТА НА КАСТОМНУЮ MODBUS FN
 int modbus_receive_raw_confirmation(modbus_t *ctx, uint8_t *rsp);
// КОНЕЦ ДОБАВЛЕНИЯ ЧАСТИ ДЛЯ ИСПРАВЛЕНИЯ БАГА ПОЛУЧЕНИЯ ОТВЕТА НА КАСТОМНУЮ MODBUS FN
