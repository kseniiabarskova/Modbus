...
int modbus_send_raw_request(modbus_t *ctx, uint8_t **raw_req, size_t raw_req_length)
{
    sft_t sft;
    ...
    return send_msg(ctx, raw_req, req_length);
}
...
// ДОБАВЛЕННАЯ ЧАСТЬ ДЛЯ ИСПРАВЛЕНИЯ БАГА ПРИ ОТПРАВКИ КАСТОМНОЙ MODBUS FN
// ИСТОЧНИК: https://github.com/stephane/libmodbus/issues/343
...
// КОНЕЦ ДОБАВЛЕННАЯ ЧАСТИ ДЛЯ ИСПРАВЛЕНИЯ БАГА ПРИ ОТПРАВКИ КАСТОМНОЙ MODBUS FN
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type, size_t *msg_size)
{
    int rc;
    fd_set rset;
    struct timeval tv;
    ...
    // Считывает те данные, которые могли остаться на сокете.
    // ВСЕГО считываем данных с сокета не более MAX_MESSAGE_LENGTH в сам буфер, который мы выводим на логи
    rc = ctx->backend->recv(ctx, msg + msg_length, MAX_MESSAGE_LENGTH - msg_length);
    if (msg_size != NULL) *msg_size = rc != -1 ? msg_length + rc : msg_length;

    // Если остались ещё данные на сокете, то делаем flush
    // Modbus Flush работает, если в сокете остается мусор, но нужно в логи выводить весь буфер
    modbus_flush(ctx);
                 ...
                 int req_length, modbus_mapping_t *mb_mapping)
{
    ...
    case MODBUS_FC_READ_DISCRETE_INPUTS: {
        ...
        if (nb < 1 || MODBUS_MAX_READ_BITS < nb) {
            rsp_length = response_exception(
                ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE, rsp, _TRUE_,
                "Illegal nb of values %d in %s (max %d)\n",
                nb, name, MODBUS_MAX_READ_BITS);
        } else if (mapping_address < 0 || (mapping_address + nb) > nb_bits) {
            rsp_length = response_exception(
                ctx, &sft,
                MODBUS_EXCEPTION_ILLEGAL_DATA_ADDRESS, rsp, _FALSE_,
    ...
    case MODBUS_FC_READ_INPUT_REGISTERS: {
        ...
        if (nb < 1 || MODBUS_MAX_READ_REGISTERS < nb) {
            ...
            rsp_length = response_exception(
                ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_ADDRESS, rsp, _FALSE_,
                "Illegal data address 0x%0X in %s\n",
                mapping_address < 0 ? address : address + nb, name);
        } else {
            int i;
    ...
    case MODBUS_FC_WRITE_SINGLE_COIL: {
        ...
        if (mapping_address < 0 || mapping_address >= mb_mapping->nb_bits) {
            ...
            if (data == 0xFF00 || data == 0x0) {
                mb_mapping->tab_bits[mapping_address] = data ? _ON_ : _OFF_;
                memcpy(rsp, req, req_length);
                rsp_length = req_length;
            } else {
                rsp_length = response_exception(
                    ctx, &sft,
                    MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE, rsp, _FALSE_,
    ...
    default:
        rsp_length = response_exception(
            ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_FUNCTION, rsp, _TRUE_,
    ...
    if (rc > 0) {
        ...
        for (i = offset; i < offset_end; i++) {
            ...
            for (bit = 0x01; (bit & 0xff) && (pos < nb);) {
                dest[pos++] = (temp & bit) ? _TRUE_ : _FALSE_;
                bit = bit << 1;
            }
...
/* Reads the boolean status of bits and sets the array elements
   in the destination to _TRUE_ or _FALSE_ (single bits). */
...
//    *request = malloc(sizeof(uint8_t) * _MIN_REQ_LENGTH);
//    *response = malloc(sizeof(uint8_t) * MAX_MESSAGE_LENGTH);
  ...
  uint8_t *req = malloc(sizeof(uint8_t) * MAX_MESSAGE_LENGTH);
  uint8_t *rsp = malloc(sizeof(uint8_t) * MAX_MESSAGE_LENGTH);

    req_length = ctx->backend->build_request_basis(ctx, function, addr, nb, req);


    rc = send_msg(ctx, &req, req_length);
//    if (rc == -1) errno = EMBTIMEOUTERR;
  if (request_size != NULL && request != NULL) {
    if (rc == -1) *request_size = 0;
    else *request_size = rc;
    ...
    if (rc > 0) {
        ...
        rc = _modbus_receive_msg(ctx, rsp, MSG_CONFIRMATION, response_size);
                          ...
                          uint8_t **response, size_t *response_size)
{
    ...
    status = read_registers(ctx, MODBUS_FC_READ_HOLDING_REGISTERS,
                            addr, nb, dest, request, request_size, response, response_size);
    return status;
}
...
int modbus_write_bit(modbus_t *ctx, int addr, int status)
{
    ...
    return write_single(ctx, MODBUS_FC_WRITE_SINGLE_COIL, addr,
                        status ? 0xFF00 : 0, NULL, NULL, NULL, NULL);
}
...
int modbus_write_registers(modbus_t *ctx, int addr, int nb, const uint16_t *src,
                           uint8_t **request, size_t *request_size, uint8_t **response, size_t *response_size)
{
    int rc;
    int req_length;
    int byte_count;
